...
interface AuthContextType {
  ...
  register: (email: string, password: string, invite_code?: string, extra?: any) => Promise<void>;
  logout: () => void;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const register = async (email: string, password: string, invite_code?: string, extra?: any) => {
    const data = await authApi.register(email, password, invite_code, extra);
    localStorage.setItem('tokup_token', data.token);
    await refreshUser();
